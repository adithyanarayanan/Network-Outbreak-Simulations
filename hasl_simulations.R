...
# numnetworks = 1 
# In Haslmere Simulations, there is only one network, amn emperical network collected from a real-world community.
# Hence, the number of networks is constant. We only vary how many outbreaks to simulate on the network.
numoutbreaks = 100
...
## Load the data file am_list.RData from the covidhm package- adjust this path appropriately
load("/cloud/project/covidhm-master/data-raw/am_list.RData")
...
ms = seq(1:1) # Tracker for current network - since there is only one network this is just a list with one number
...
for(m in ms){
  ...
  # Size of this network is different, and unchangeable.
  nodes = 411
  A <- am_list[[1]]
  A = as.matrix((A>0)+0)
  
  # Load graph
  kgraph <- graph_from_adjacency_matrix(A,mode = "undirected")
  
  # Delete unconnected nodes  
  uncon = which(degree(kgraph) == 0)
  kgraph = delete.vertices(kgraph, uncon)
  ...
  num = 103
  top_priorities <- match(head(sort(hub_weights,decreasing = T),num),hub_weights) # stores the 103 most influential nodes in our generated network
