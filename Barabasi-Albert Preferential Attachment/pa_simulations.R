...
## Task: Simulate outbreaks, and augmented novel tracing strategies, on Preferential Attachment Networks with 500 nodes.
...
ms = seq(1:numnetworks) # Tracker for current network (m'th network)
...
for(m in ms){
  ...
  set.seed(m)
  kgraph = sample_pa(500, power=1, m=4, directed = FALSE, out.pref = TRUE)
  nodes = 500
